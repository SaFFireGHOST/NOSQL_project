...
def merge_logs(local_log_file, remote_log_file):
    ...
    # Convert to final hashmap: (student_id, course_id) -> grade, only for remote updates
    result = {key: grade for key, (timestamp, grade, source) in latest_updates.items() if source == 'remote'}
...
def merge_mongo(database_system):
    ...
    try:
        ...
        # Apply updates
        updated_count = 0
        for (student_id, course_id), grade in updates.items():
            query = {'student-ID': student_id, 'course-id': course_id}
            update = {'$set': {'grade': grade}}
            result = collection.update_one(query, update)
            if result.matched_count > 0:
                updated_count += 1
                log_operation(MONGO_LOG, 'SET', student_id, course_id, grade)
...
def merge_mysql(database_system):
    ...
    try:
        ...
        for (student_id, course_id), grade in updates.items():
            ...
            if cursor.rowcount > 0:
                updated_count += 1
                log_operation(MYSQL_LOG, 'SET', student_id, course_id, grade)
    ...
    try:
        ...
        for (student_id, course_id), grade in updates.items():
            ...
            if cursor.fetchone()[0] > 0:
                updated_count += 1
                log_operation(HIVE_LOG, 'SET', student_id, course_id, grade)
